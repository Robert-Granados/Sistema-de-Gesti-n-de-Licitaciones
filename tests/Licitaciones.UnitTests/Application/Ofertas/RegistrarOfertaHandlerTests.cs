...
public sealed class RegistrarOfertaHandlerTests
{
    ...
    [Fact]
    public async Task Handle_MontoMayorAlPresupuesto_Rechaza()
    {
        var licitacionId = Guid.NewGuid();
        var validacion = new FakeValidacionRepository(
            licitacionId: licitacionId,
        ...
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
            () => handler.HandleAsync(
                new RegistrarOfertaCommand(licitacionId, Guid.NewGuid(), 200_000m)));
    }

    [Fact]
    public async Task Handle_MontoIgualAlPresupuesto_Acepta()
    {
        var licitacionId = Guid.NewGuid();
        var proveedorId = Guid.NewGuid();
        var validacion = new FakeValidacionRepository(
        ...
        var result = await handler.HandleAsync(
            new RegistrarOfertaCommand(licitacionId, proveedorId, 1_000_000m));
    ...
    private sealed class FakeValidacionRepository(
        ...
        bool yaTieneOferta) : IOfertaValidacionRepository
    {
        ...
        public Task<bool> YaTieneOfertaAsync(
            ...
            Task.FromResult(yaTieneOferta);
    }

    private sealed class FakeWriteRepository : IOfertaWriteRepository
    {
